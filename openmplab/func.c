...
{
	int i;
    double weight, dxr, dyr;
    weight = 1/((double)(n));
    dxr = xr;
    dyr = yr;
#pragma omp parallel for num_threads(29) private(i)
    for(i = 0; i < n; i++){
        weights[i] = weight;
        arrayX[i] = dxr;
        arrayY[i] = dyr;
...
#pragma omp parallel for num_threads(29) private (i, j, index_X, index_Y)
   	for(i = 0; i<n; i++){
        for(j = 0; j < Ones; j++){
...
void func5(double *x_j, double *y_j, double *arrayX, double *arrayY, double *weights, double *cfd, double *u, int n)
{
    ...
    int n_minus_one;
    n_minus_one = n - 1;
#pragma omp parallel for num_threads(29) private(i, j)
    for(j = 0; j < n; j++){
